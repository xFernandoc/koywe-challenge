...
describe('Quote service', () => {
  ...
  describe('Crear una cotización', () => {
    ...
    it('Crear una cotizacion con error por indice duplicado', async () => {
      ...
      await expect(quoteService.createQuote(mockQuoteTemplate)).rejects.toThrow(
        'Error, indice duplicado',
      );
    });
  });
  ...
  describe('Validar cotizacion', () => {
    ...
    it('Cotización no encontrada', async () => {
      repository.findById.mockResolvedValue(null);
      await expect(quoteService.getValidQuoteByID('id-bad')).rejects.toThrow(
        NotFoundException,
      );
      await expect(quoteService.getValidQuoteByID('id-bad')).rejects.toThrow(
    ...
    it('Cotización expirada', async () => {
      ...
      await expect(
        quoteService.getValidQuoteByID(mockExpired.id),
      ).rejects.toThrow(NotFoundException);
      await expect(
        quoteService.getValidQuoteByID(mockExpired.id),
      ).rejects.toThrow('Cotización expirada');
    });
  });
  ...
  describe('Validar propiedad de cotizacion', () => {
    it('Cotización pertenece a usuario', async () => {
      ...
      const resultQuote = await quoteService.validQuoteOwner(
        mockQuoteTemplate.id,
        mockQuoteTemplate.user as string,
      );
      expect(resultQuote).toEqual(mockQuoteTemplate);
      expect(repository.findByIdAndUser).toHaveBeenCalledWith(
        mockQuoteTemplate.id,
        mockQuoteTemplate.user,
      );
    });
    it('Cotización no pertenece a usuario', async () => {
      repository.findByIdAndUser.mockResolvedValue(null);
      await expect(
        quoteService.validQuoteOwner(mockQuoteTemplate.id, 'id-bad'),
      ).rejects.toThrow(UnauthorizedException);
      await expect(
        quoteService.validQuoteOwner(mockQuoteTemplate.id, 'id-bad'),
      ).rejects.toThrow('No tienes acceso a esta cotización');
      expect(repository.findByIdAndUser).toHaveBeenCalledWith(
        mockQuoteTemplate.id,
        'id-bad',
      );
  ...
  describe('Servicios adicionales', () => {
    it('Validando calculo del valor convertido', () => {
      ...
      const calculateConvertValue = quoteService.calculateConvertValue(
        amount,
        rate,
      );
    ...
    it('validando creación de entity quote', () => {
      ...
      const quoteEntity = quoteService.createEntityQuote(
        createQuoteDTO,
        'user-id',
        2.5,
      );
