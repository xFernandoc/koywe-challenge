...
describe('Quote Facade', () => {
  ...
  const listCurrency = ['USDT', 'PEN', 'BTC'];
  ...
  describe('Registrar cotizacion', () => {
    it('Crear cotizacion correctamente', async () => {
      ...
      const resultQuote = await facade.registerQuote(
        mockCreateQuoteDTO,
        'userid',
      );

      expect(resultQuote).toEqual(mockQuoteTemplate);
      expect(exchangeProvider.getPrice).toHaveBeenCalledWith(
        mockCreateQuoteDTO,
      );
      expect(quoteService.createEntityQuote).toHaveBeenCalledWith(
        mockCreateQuoteDTO,
        'userid',
        parseFloat(mockResulCurrency.price),
      );
    ...
    it('Crear cotizacion pero no existe moneda', async () => {
      ...
      await expect(
        facade.registerQuote(mockCreateQuoteDTO, 'userid'),
      ).rejects.toThrow(NotFoundException);
      await expect(
        facade.registerQuote(mockCreateQuoteDTO, 'userid'),
      ).rejects.toThrow('Moneda de busqueda no existe');
    });
  });
  ...
  describe('Obtener cotizacion', () => {
    ...
    it('Obtener una cotizacion vencida', async () => {
      quoteService.getValidQuoteByID.mockRejectedValue(
        new NotFoundException('Cotización no existe'),
      );
      await expect(facade.getQuote(mockQuoteTemplate.id)).rejects.toThrow(
        NotFoundException,
      );
      await expect(facade.getQuote(mockQuoteTemplate.id)).rejects.toThrow(
    ...
    it('Obtener una cotizacion que no existe', async () => {
      quoteService.getValidQuoteByID.mockRejectedValue(
        new NotFoundException('Cotización expirada'),
      );
      await expect(facade.getQuote(mockQuoteTemplate.id)).rejects.toThrow(
        NotFoundException,
      );
