...
describe('Exchange provider',() => {
    ...
    describe('Obteniendo precio',() => {
        ...
        it('Obteniendo el precio de una moneda inválida',async() => {
            ...
            const responseCurrency = await exchangeProvider.getPrice(mockCreateQuoteDTO);
        ...
        it('Obteniendo el precio de una moneda que no existe',async() => {
            ...
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(BadRequestException)
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow('No existe precio para esta combinación de moneda.')
        })
        ...
        it('Mostrar errores axios de api 2002',() => {
            ...
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(BadRequestException)
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(`Moneda ${currencyError} no existe, consulta en /quote/currency-available`)
        })
        it('Mostrar errores axios de api no conocido',() => {
            ...
            httpService.get.mockReturnValue(throwError(() => axiosError))
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(BadRequestException)
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(`Error desconocido`)
        })
        it('Mostrar errores no axios de api no conocido',() => {
            httpService.get.mockReturnValue(throwError(() => new Error('No conocido')))
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(InternalServerErrorException)
            expect(exchangeProvider.getPrice(mockCreateQuoteDTO)).rejects.toThrow(`Error desconocido`)
        })
    })
    ...
    describe('Obteniendo monedas',() => {
        ...
        it('Mostrar errores axios de api no conocido',() => {
            ...
            httpService.get.mockReturnValue(throwError(() => axiosError))
            expect(exchangeProvider.getCurrencys()).rejects.toThrow(BadRequestException)
            expect(exchangeProvider.getCurrencys()).rejects.toThrow(`Error desconocido`)
        })
        it('Mostrar errores no axios de api no conocido',() => {
            httpService.get.mockReturnValue(throwError(() => new Error('No conocido')))
            expect(exchangeProvider.getCurrencys()).rejects.toThrow(InternalServerErrorException)
            expect(exchangeProvider.getCurrencys()).rejects.toThrow(`Error desconocido`)
        })
    })
})