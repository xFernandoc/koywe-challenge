...
export class AuthFacade {
  ...
  async create(createUserDTO: CreateUserDTO) {
    const getUserExist = await this.userService.getUserByEmail(createUserDTO.email);
    if(getUserExist) {
        throw new BadRequestException('User already exists');
    }
    const passwordHash = await this.userService.generatePasswordHash(createUserDTO.password);
